...
var DestinationsSchema = require('../models/destinations.js').destinationsSchema; // load schema
var Destinations = db.model('destinations', DestinationsSchema); // Load data model for schema
...
function destinationSaveResult(err, doc) {
    if (err) {
        console.log(err);
    }
}
...
Destinations.remove({}, function(err) {
    var i = 0;
    while (i < destinations.length) {
        new Destinations(destinations[i]).save(destinationSaveResult);
        i++;
    }
});
...
exports.findAll = function(req, res) {
    ...
    Destinations.find({}, function(error, items) {
        res.send(items);
    });
};

exports.addDestination = function(req, res) {
    var destinationObj = req.body;
    console.log('Adding destination: ' + JSON.stringify(destinationObj));

    var destination = new Destination(destinationObj);
    destination.save(function(err, doc) {
        if (err || !doc) {
            throw err;
        } else {
...
exports.updateDestination = function(req, res) {
    ...
    Model.update(query, destination, options, function(err, numberAffected, raw) {
        if (err) {
...
exports.deleteDestination = function(req, res) {
    ...
    Poll.findByIdAndRemove(destinationId, function(err, doc) {
        if (err || !doc) {
            throw err;
        } else {
